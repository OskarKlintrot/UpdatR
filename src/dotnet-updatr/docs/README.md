...
Source File: /src/dotnet-updatr/docs/README.source.md
...
See [UpdatR](https://www.nuget.org/packages/UpdatR/) for SDK.
...
UpdatR is used to update it's own dependencies, have a look at [Build.cs](https://github.com/OskarKlintrot/UpdatR/blob/main/tools/Build/Build.cs) for an example that uses [Bullseye](https://www.nuget.org/packages/Bullseye) and [SimpleExec](https://www.nuget.org/packages/SimpleExec). However, if you are using C# in your CI/CD pipeline it's probably easier to just use [UpdatR](https://www.nuget.org/packages/UpdatR/) directly instead. That's the package that powers `dotnet-updatr` under the hood.
